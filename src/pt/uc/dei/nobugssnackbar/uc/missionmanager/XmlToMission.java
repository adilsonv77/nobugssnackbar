...
public class XmlToMission {
	...
	private static String conditionRegex = "(?<funcName>\\w*\\.*\\w+)\\((?<param>\\'*\\w+\\'*)*\\)\\s*(?<comparator>[!<>=]{1,2})*\\s*(?<value>\\'*\\w*\\.*\\w+\\'*)*\\s*(?<logOp>(\\&\\&)|(&amp;&amp;)|(\\|\\|))*";
	...
	public static List<Condition> stringConditionToObject(String conditionAsString) {
		...
		conditionAsString = conditionAsString.replace("&lt;", "<").replace("&gt;", ">").replace("&amp;", "&");
		
		Pattern pattern = Pattern.compile(conditionRegex);
		Matcher matcher = pattern.matcher(conditionAsString);
		while (matcher.find()) {
			Condition cond = new Condition();
			Function func = new Function();
			func.setName(matcher.group("funcName").replace("Hints.", ""));
			cond.setFunction(func);
			FunctionValue funcVal = new FunctionValue();
			funcVal.setName(matcher.group("param"));
			if (funcVal.getName() != null) {
				funcVal.setName(funcVal.getName().replace("\'", ""));
				funcVal.setParam(true);
			}
			cond.setParameter(funcVal);
			cond.setComparator(matcher.group("comparator"));
			...
			cond.setValue(val);
			list.add(cond);
			String logOp = matcher.group("logOp");
			if (logOp != null) {
				Condition condOp = new Condition();
				condOp.setLogicalOperator(logOp);
				list.add(condOp);
			}
		}
