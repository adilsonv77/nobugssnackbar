...
SnackMan = function(hasTable, objectives, mission, avatar) {
	...
	this.table = [this.snackManFinalPath[10], this.snackManFinalPath[9]];
	...
	var coffee = mission.evaluate("count(//mission/commands/category[@name='coffee'])", mission, null,  XPathResult.ANY_TYPE, null);
	this.showCoffee = coffee.numberValue > 0; 
	if (!this.showCoffee) {
		coffee = mission.evaluate("count(//mission/xml//block[@type='move_goToCoffeeMachine'])", mission, null,  XPathResult.ANY_TYPE, null);
		this.showCoffee = coffee.numberValue > 0; 
	}
	
	if (this.showCoffee) {
		this.coffeeMachine = new Sprite({
			ticksPerFrame: 0,
			numberOfFrames: 4,
			horzSeq: true,
			x: 130,
			...
			sourceX: 0,
			sourceY: 0,
			img : PreloadImgs.get("coffeemachine")
		});
   ...
   if (hasTable){
    	...
    	this.cad2 = PreloadImgs.get("chair2");
    }
    else
    	this.mesa = null;
...
SnackMan.prototype.reset = function() {
	...
	if (this.coffeeMachine != null)
		this.coffeeMachine.frameIndex = 0;
...
SnackMan.prototype.draw = function(ctx) {
	...
	if (this.mesa != null) {
		...
		ctx.drawImage(this.mesa, 200, 73);
		ctx.drawImage(this.cad1, 160, 80);
		ctx.drawImage(this.cad2, 280, 80);
	}
...
SnackMan.prototype.goToTable = function(cust) {
	...
	this.animateSnackMan( this.table[cust-1] );
...
SnackMan.prototype.getCustomer = function() {
	...
	for (var i=0; i<this.table.length;i++)
		if (this.currentNode.id === this.table[i].id) {
			return (CustomerManager.getCustomerTable(i+1));
		}
...
SnackMan.prototype.pickUpFruits = function(order) {
	...
	if (order_data === "\"$$$juiceoforange\"") {
		// prepare a juice even there is no customer order
	...
	// does the order have the food of this place ?
	if (!(order_data.type === "order" && order_data.descr.indexOf("$$juiceof") == 0)) {
	...
	// TODO in future version, maybe the display has limited stock
	var item = {type: "item", descr:"$$"+order_data.descr.substring(9), drinkOrFood: "drink", source: order_data.source, sourceType: order_data.sourceType};
...
SnackMan.prototype.goToCoffeeMachine = function() {
	
	this.animateSnackMan( this.coffeeMachineNode );  
};

SnackMan.prototype.prepareCoffee = function() {
	
	if (this.currentNode.id != this.coffeeMachineNode.id) {
		BlocklyApps.log.push(["fail", "Error_isntFrontCoffeeMachine"]);
		throw false;
	}
	
	if (this.coffeeMachine.frameIndex > 0) {
		BlocklyApps.log.push(["fail", "Error_stillHaveCoffee"]);
		throw false;
	}
...
SnackMan.prototype.pickUpCoffee = function(order) {
	...
	if (this.coffeeMachine.frameIndex == 0) {
		BlocklyApps.log.push(["fail", "Error_doesntHaveCoffee"]);
		throw false;
	}
	...
	// does the order have the food/drink of this place ?
	if (order.data.descr != "$$coffee") {
		BlocklyApps.log.push(["fail", "Error_onlyPickUpCoffee"]);
		throw false;
	}
	
	
	if (this.coffeeMachine.frameIndex == 2) {
		this.update('CoM', 3);
	} else {
		this.update('CoM', 0);
	}
	...
	var item = {type: "item", descr:"$$coffee", drinkOrFood: "drink", source: order.data.source, sourceType: order.data.sourceType};
	//this.verifyObjectives("pickupCoffee", item);
	this.catched++;
	...
	if (command !== "IM") // the sequence of talk is TA and IM. So, if come any other command, can reset the text
	  ...
	  switch (command) {
	    ...
	    case 'CO':
	 	...
	 	case 'PC':
	 		this.nexPrepareCoffee();
	 		break;

	 	case 'CoM':
	 		this.nexPrepareCoffee(values.shift());
	  }
...
SnackMan.prototype.nexPrepareCoffee = function(index) {
	if (index)
		this.coffeeMachine.frameIndex = index;
	else
